...
When `dssr_background_updates=true`, additional bound snapshots are published during solve
...
| `--dssr_background_updates true` | false in `parallel_mode=deterministic`, true in `parallel_mode=opportunistic` | Run background DSSR bound tightening during B&B |
| `--dssr_background_policy <fixed\|auto>` | `fixed` | Async DSSR scheduling policy; `auto` stops early when search/checkpoint activity is low or DSSR stops improving bounds/incumbent |
| `--dssr_background_max_epochs N` | 0 | Hard cap on async DSSR stages (`0` = uncapped) |
