...
TEST_CASE("edge_elimination with infinite UB eliminates nothing",
          "[elimination]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
  auto fwd = cptp::preprocess::forward_labeling(prob, prob.depot());
  auto bwd = fwd;
...
TEST_CASE("edge_elimination with very tight UB eliminates edges",
          "[elimination]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
  auto fwd = cptp::preprocess::forward_labeling(prob, prob.depot());
  auto bwd = fwd;
  ...
  auto eliminated =
      cptp::preprocess::edge_elimination(prob, fwd, bwd, tight_ub, correction);
...
TEST_CASE("labeling_from with problem costs matches default overload",
          "[labeling]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
...
TEST_CASE("labeling_from with zero costs gives non-positive bounds",
          "[labeling]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
  ...
  auto bounds = cptp::preprocess::labeling_from(prob, prob.depot(), zero_costs,
                                                prob.profits());
...
TEST_CASE("labeling_from with scaled costs gives scaled bounds", "[labeling]") {
  ...
  for (int32_t e = 0; e < m; ++e) scaled_costs[e] = prob.edge_cost(e) * 2.0;
  for (int32_t i = 0; i < n; ++i) scaled_profits[i] = prob.profit(i) * 2.0;

  auto original = cptp::preprocess::labeling_from(
      prob, prob.depot(), prob.edge_costs(), prob.profits());
  auto scaled = cptp::preprocess::labeling_from(prob, prob.depot(),
                                                scaled_costs, scaled_profits);
...
TEST_CASE("labeling_from with forbidden node gives higher bound",
          "[labeling]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
  const int32_t n = prob.num_nodes();
  ...
  auto bounds = cptp::preprocess::labeling_from(prob, prob.depot(),
                                                prob.edge_costs(), mod_profits);
...
TEST_CASE("labeling_from with negative costs finds shorter paths",
          "[labeling]") {
  auto prob = cptp::io::load("tests/data/tiny4.txt");
  const int32_t m = prob.num_edges();
  ...
  for (int32_t e = 0; e < m; ++e) cheap_costs[e] = prob.edge_cost(e) * 0.5;

  auto cheaper = cptp::preprocess::labeling_from(prob, prob.depot(),
                                                 cheap_costs, prob.profits());
...
TEST_CASE("Propagator does not change optimal solution on tiny4",
          "[propagator]") {
...
TEST_CASE("All-pairs propagation produces valid solution on tiny4",
  ...
  auto r =
      solve_instance("tests/data/tiny4.txt", {{"all_pairs_bounds", "true"}});
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-11.0, 1.0));
}

TEST_CASE("All-pairs and default produce same objective",
          "[propagator][all_pairs]") {
...
TEST_CASE("All-pairs on larger instance B-n45-k6-54",
  ...
  auto r = solve_instance("benchmarks/instances/spprclib/B-n45-k6-54.sppcc",
                          {{"time_limit", "60"}, {"all_pairs_bounds", "true"}});
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-74278.0, 1.0));
}
...
TEST_CASE("All-pairs propagation on path instance", "[propagator][all_pairs]") {
  auto r = solve_instance("tests/data/tiny4_path.txt",
