...
TEST_CASE(
    "Heuristic stage UB gate: ILS snapshot matches on SPPRCLIB + Roberti",
    ...
    std::cout << std::left << std::setw(32) << "Instance"
              << std::right << std::setw(14) << "Expected"
    ...
    for (const auto& path : instances) {
        ...
        // Snapshot values are stored from compact textual output (~6 significant digits).
        const double tol = 1e-3 + 1e-5 * std::abs(expected_obj);

        std::cout << std::left << std::setw(32) << key
                  << std::right << std::setw(14) << expected_obj
                  << std::setw(14) << ils.objective
                  << std::setw(14) << delta
                  << std::setw(14) << tol << "\n";
