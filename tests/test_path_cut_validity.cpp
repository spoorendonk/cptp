...
EquivalentInstances make_equivalent_instances() {
    ...
    edges_path.push_back({0, 5});      // keep terminals tightly connected in support graph
...
EquivalentInstances make_comb_equivalent_instances() {
    ...
    path.build(8, edges_path, costs_path, profits_path, demands_path, 100.0, 0, 7);
    return {std::move(tour), std::move(path)};
}
...
EquivalentSample make_comb_equivalent_sample(const EquivalentInstances& inst) {
    ...
    // Keep them below Comb BFS thresholds (0.3/0.5) so outside nodes stay outside H.
    set_tour(1, 4, 0.25); set_path(1, 4, 0.25);
    set_tour(2, 5, 0.25); set_path(2, 5, 0.25);
    set_tour(3, 6, 0.25); set_path(3, 6, 0.25);

    return sample;
}

std::vector<EquivalentSample> build_equivalent_samples(const EquivalentInstances& inst,
                                                       int num_samples) {
    ...
    for (int s = 0; s < num_samples; ++s) {
        ...
        for (int i = 1; i <= 4; ++i) {
            for (int j = i + 1; j <= 4; ++j) {
                const double b = (u01(rng) < 0.6) ? (0.55 + 0.45 * u01(rng)) : (0.02 * u01(rng));
    ...
    // Crafted sample to trigger comb-like handle/teeth structure.
    {
        ...
        for (auto e : g.edges()) {
            ...
            if ((u == 0 && (v == 1 || v == 2 || v == 3)) ||
                (v == 0 && (u == 1 || u == 2 || u == 3))) {
                x[e] = 0.85;
            } else if ((u == 1 && v == 4) || (u == 2 && v == 4) || (u == 3 && v == 4) ||
                       (u == 1 && v == 5) || (u == 2 && v == 5) || (u == 3 && v == 5)) {
                x[e] = 0.75;
            } else {
                x[e] = 0.08;
            }
...
void validate_separator_on_path(const cptp::Problem& prob,
                                ...
                                bool require_generated_cuts = true) {
    const int32_t m = prob.num_edges();
    ...
    for (const auto& [x_values, y_values] : samples) {
        ...
        for (const auto& cut : cuts) {
            for (const auto& path_sol : feasible_paths) {
                const double lhs = evaluate_cut_lhs(cut, path_sol.x_values, path_sol.y_values);
...
TEST_CASE("Path cut families generate only valid cuts on all feasible s-t paths",
...
TEST_CASE("Equivalent tour and s-t path trigger same cut families", "[separators][path][equiv]") {
    ...
    // Customer subsets over {1,2,3,4}. In path model, 0=source and 5=target are terminals.
    for (const auto& sample : samples) {
        for (int mask = 1; mask < (1 << 4); ++mask) {
            ...
            const auto st_tour = compute_set_stats(inst.tour, sample.x_tour, sample.y_tour, in_tour);
            const auto st_path = compute_set_stats(inst.path, sample.x_path, sample.y_path, in_path);
            ...
            if (Q_r > tol) {
                ...
                if (k > 1.0) {
                    const double rci_tour = 2.0 * (k - st_tour.dS / Q_r)
                        - (st_tour.cut_flow - (2.0 * st_tour.served_demand) / Q_r);
                    const double rci_path = 2.0 * (k - st_path.dS / Q_r)
            ...
            if (r > tol) {
                ...
                if (k > 1.0) {
                    const double beta_tour =
                        (st_tour.dS - st_tour.served_demand) +
                        2.0 * (d_total - st_tour.dS) - st_tour.star_sum;
                    const double beta_path =
                        (st_path.dS - st_path.served_demand) +
                        2.0 * (d_total - st_path.dS) - st_path.star_sum;
                    const double rglm_tour = (2.0 * k - 2.0 * beta_tour / r) - st_tour.cut_flow;
                    const double rglm_path = (2.0 * k - 2.0 * beta_path / r) - st_path.cut_flow;
...
TEST_CASE("Comb path emits valid cuts on crafted sample", "[comb][path][validity]") {
...
TEST_CASE("Comb expression and separator behavior match across equivalent tour/path",
          "[comb][path][equiv]") {
    const auto inst = make_comb_equivalent_instances();
    const auto sample = make_comb_equivalent_sample(inst);
    ...
    H_tour[0] = 1; H_path[0] = 1;
    H_path[inst.path.target()] = 1;
    for (int i : {1, 2, 3}) {
        H_tour[i] = 1;
        H_path[i] = 1;
    }
    std::vector<std::pair<int32_t, int32_t>> teeth = {
        {1, 4}, {2, 5}, {3, 6}
    };
    const double lhs_tour = compute_comb_lhs(inst.tour, sample.x_tour, sample.y_tour, H_tour, teeth);
    ...
    REQUIRE(std::abs(max_violation(cuts_tour) - max_violation(cuts_path)) <= 1e-6);
}

TEST_CASE("Comb path ignores split-terminal artifact variables", "[comb][path][invariance]") {
    const auto inst = make_comb_equivalent_instances();
    ...
    const auto sig_y_target = normalize_cut_signatures(run_comb_path(y_target_perturbed));
