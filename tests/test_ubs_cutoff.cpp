...
TEST_CASE("prove-only: B-n45-k6-54", "[prove][slow]") {
  auto r = prove_instance("benchmarks/instances/spprclib/B-n45-k6-54.sppcc",
                          -74278.0, 60);
  REQUIRE(r.is_optimal());
...
TEST_CASE("prove-only: B-n50-k8-40", "[prove][slow]") {
  auto r = prove_instance("benchmarks/instances/spprclib/B-n50-k8-40.sppcc",
                          -12832.0, 60);
  REQUIRE(r.is_optimal());
...
TEST_CASE("prove-only: B-n52-k7-15", "[prove][slow]") {
  auto r = prove_instance("benchmarks/instances/spprclib/B-n52-k7-15.sppcc",
                          -74998.0, 60);
  REQUIRE(r.is_optimal());
...
TEST_CASE("prove-only: A-n63-k10-44", "[prove][slow]") {
  auto r = prove_instance("benchmarks/instances/spprclib/A-n63-k10-44.sppcc",
                          -32561.0, 60);
  REQUIRE(r.is_optimal());
...
TEST_CASE("prove-only: A-n69-k9-42", "[prove][slow]") {
  auto r = prove_instance("benchmarks/instances/spprclib/A-n69-k9-42.sppcc",
                          -43290.0, 60);
  REQUIRE(r.is_optimal());
...
static cptp::SolveResult prove_rc_fixing(const char* path, double cutoff,
  ...
  if (fix_to_one) opts.push_back({"rc_fixing_to_one", "true"});
  return model.solve(opts);
}
...
TEST_CASE("rc_fixing on_ub_improvement: tiny4 tour optimal",
          "[prove][rc_fixing]") {
...
TEST_CASE("rc_fixing with fix_to_one: tiny4 tour optimal",
          "[prove][rc_fixing]") {
...
TEST_CASE("rc_fixing with fix_to_one: tiny4 path optimal",
          "[prove][rc_fixing]") {
  auto r =
      prove_rc_fixing("tests/data/tiny4_path.txt", -13.0, "root_only", true);
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-13.0, 1.0));
}

TEST_CASE("rc_fixing on_ub_improvement: tiny4 path optimal",
          "[prove][rc_fixing]") {
  auto r =
      prove_rc_fixing("tests/data/tiny4_path.txt", -13.0, "on_ub_improvement");
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-13.0, 1.0));
}

TEST_CASE("rc_fixing off matches on_ub_improvement on tiny4",
          "[prove][rc_fixing]") {
  auto r_off = prove_rc_fixing("tests/data/tiny4.txt", -11.0, "off");
  auto r_default = prove_instance("tests/data/tiny4.txt", -11.0,
                                  30);  // default = on_ub_improvement
  REQUIRE(r_off.is_optimal());
  REQUIRE(r_default.is_optimal());
  REQUIRE_THAT(r_off.objective, WithinAbs(r_default.objective, 1e-6));
}

TEST_CASE("rc_fixing all strategies agree on optimum: tiny4",
          "[prove][rc_fixing]") {
  auto r_root = prove_rc_fixing("tests/data/tiny4.txt", -11.0, "root_only");
  auto r_ub =
      prove_rc_fixing("tests/data/tiny4.txt", -11.0, "on_ub_improvement");
...
TEST_CASE("rc_fixing default active in normal solve: tiny4 path",
          "[rc_fixing]") {
...
TEST_CASE("rc_fixing on_ub_improvement + fix_to_one: tiny4 path",
          "[prove][rc_fixing]") {
  auto r = prove_rc_fixing("tests/data/tiny4_path.txt", -13.0,
...
TEST_CASE("rc_fixing root_only: B-n45-k6-54", "[prove][rc_fixing][slow]") {
  auto r = prove_rc_fixing("benchmarks/instances/spprclib/B-n45-k6-54.sppcc",
                           -74278.0, "root_only", false, 60);
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-74278.0, 1.0));
}

TEST_CASE("rc_fixing on_ub_improvement: B-n45-k6-54",
          "[prove][rc_fixing][slow]") {
  auto r = prove_rc_fixing("benchmarks/instances/spprclib/B-n45-k6-54.sppcc",
                           -74278.0, "on_ub_improvement", false, 60);
  REQUIRE(r.is_optimal());
  REQUIRE_THAT(r.objective, WithinAbs(-74278.0, 1.0));
}
