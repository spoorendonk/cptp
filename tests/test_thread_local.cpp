...
TEST_CASE("Separator: hasCallback is false initially",
          "[thread_local][separator]") {
  HighsUserSeparator::clearCallback();
  REQUIRE_FALSE(HighsUserSeparator::hasCallback());
}

TEST_CASE("Separator: setCallback / hasCallback / clearCallback",
          "[thread_local][separator]") {
  HighsUserSeparator::clearCallback();
  REQUIRE_FALSE(HighsUserSeparator::hasCallback());
...
TEST_CASE("Propagator: hasCallback is false initially",
          "[thread_local][propagator]") {
  HighsUserPropagator::clearCallback();
  REQUIRE_FALSE(HighsUserPropagator::hasCallback());
}

TEST_CASE("Propagator: setCallback / hasCallback / clearCallback",
          "[thread_local][propagator]") {
  HighsUserPropagator::clearCallback();
  REQUIRE_FALSE(HighsUserPropagator::hasCallback());
...
TEST_CASE("Separator: callbacks are thread-local isolated",
  ...
  // Cross-contamination check: A's callback was never called by B and vice
  // versa
