...
TEST_CASE(
    "Model tour allows single-customer cycle with depot-edge multiplicity",
    "[model][tour][depot_x2]") {
  cptp::Model model;
...
TEST_CASE(
    "Initial heuristic allows single-customer tour with x=2 on depot edge",
  ...
  REQUIRE(start.col_values.size() == static_cast<size_t>(m + prob.num_nodes()));
...
TEST_CASE("Heuristic local search: swap(1,1) improves saturated seed",
          "[heuristic]") {
  ...
  REQUIRE(cptp::heuristic::detail::init_seed_state(prob, tour, in_tour,
                                                   remaining_cap));
  REQUIRE_THAT(remaining_cap, WithinAbs(0.0, 1e-12));

  const double before = cptp::heuristic::detail::tour_objective(prob, tour);
  cptp::heuristic::detail::local_search(prob, tour, in_tour, remaining_cap, {},
                                        50);
...
TEST_CASE(
    "Heuristic pool local search: deterministic across worker counts (tour)",
  ...
  auto single =
      cptp::heuristic::run_local_search_from_pool(prob, pool, starts, 200, 1);
  auto parallel =
      cptp::heuristic::run_local_search_from_pool(prob, pool, starts, 200, 4);
...
TEST_CASE(
    "Heuristic pool local search: deterministic across worker counts (path)",
...
TEST_CASE("Heuristic conversion throws on invalid route",
          "[heuristic][validation]") {
  auto prob = make_heuristic_small_tour_problem();
  // Demands: 3 + 4 + 2 > capacity 7.
  std::vector<int32_t> invalid = {0, 1, 2, 3, 0};
  REQUIRE_THROWS_AS(cptp::heuristic::detail::tour_to_solution(prob, invalid),
                    std::runtime_error);
}
...
TEST_CASE("Heuristic warm-start progress snapshots are monotonic",
  ...
  auto result = cptp::heuristic::run_local_search_from_pool(
      prob, pool, static_cast<int>(pool.candidates.size()), 200, 4, {},
      &progress_opts,
...
TEST_CASE("Heuristic warm-start reuses seeds to honor requested starts",
  ...
  const int requested_starts = static_cast<int>(pool.candidates.size()) + 5;
  cptp::heuristic::WarmStartProgressOptions progress_opts;
  progress_opts.report_every_starts = 1;
  progress_opts.report_on_ub_improvement = true;
...
TEST_CASE("Model: source==target gives same result as set_depot",
          "[model][path]") {
  // Build two identical models, one using set_depot, one using
...
TEST_CASE("Model: extended tuning options parse and preserve correctness",
          "[model][options]") {
  cptp::Model model;
...
TEST_CASE(
    "Model: deterministic work-unit settings produce stable repeated solves",
...
TEST_CASE("Model: disabling cut families still yields valid tiny solution",
          "[model][options][cuts]") {
  cptp::Model model;
...
TEST_CASE(
    "Model: connectivity still enforced when SEC cut generation is disabled",
...
TEST_CASE(
    "Model: connectivity check still enforced with SEC off and submip "
    "separation off",
    "[model][options][cuts][feasibility]") {
...
TEST_CASE("Model: edge elimination toggles preserve tiny optimum",
          "[model][options][presolve]") {
  cptp::Model model_on;
...
TEST_CASE("Model: presolve option is captured and forced off",
          "[model][options][presolve]") {
  cptp::Model model_a;
  cptp::Model model_b;
  ...
  opts_forced.push_back(
      {"presolve", "true"});  // should be intercepted and ignored
...
TEST_CASE("Model: hyperplane branching modes produce valid results",
          "[model][branching]") {
  std::vector<cptp::Edge> edges = {{0, 1}, {0, 2}, {0, 3},
                                   {1, 2}, {1, 3}, {2, 3}};
  std::vector<double> costs = {10.0, 8.0, 12.0, 6.0, 7.0, 5.0};
  ...
  double ref_obj;
  {
    ...
    if (!result.has_solution()) {
      WARN(
          "Reference run produced no incumbent; skipping per-mode objective "
          "checks");
      return;
    }
  ...
  for (const char* mode :
       {"pairs", "clusters", "demand", "cardinality", "all"}) {
...
TEST_CASE("Model: mip_max_nodes limit is surfaced as non-error status",
          "[model][limits]") {
  cptp::Model model;
  std::vector<cptp::Edge> edges = {{0, 1}, {1, 2}, {0, 2}, {2, 3}, {1, 3}};
  ...
  if (r.status == cptp::SolveResult::Status::Error) {
    WARN(
        "mip_max_nodes run returned Error in this ordering; skipping strict "
        "assertion");
    return;
  }
}
