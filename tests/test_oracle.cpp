...
TEST_CASE("SeparationOracle: max_cuts_per_separator limits output",
          "[oracle]") {
  auto prob = make_small_problem();
  int32_t m = prob.num_edges();
  int32_t n = prob.num_nodes();
...
TEST_CASE("SeparationOracle: path mode works", "[oracle][path]") {
  ...
  for (auto e : graph.edges()) {
    int32_t u = graph.edge_source(e);
    int32_t v = graph.edge_target(e);
    if ((u == 0 && v == 3) || (u == 1 && v == 2)) x_values[e] = 1.0;
  }
...
TEST_CASE("BoundPropagator: propagate_fixed_edge (neighbor-only)",
          "[propagator]") {
...
TEST_CASE("SeparationOracle: add_default_separators is cumulative",
          "[oracle]") {
  auto prob = make_small_problem();
...
TEST_CASE("SeparationOracle: is_feasible works in path mode",
          "[oracle][path]") {
  auto prob = make_small_path_problem();
  int32_t m = prob.num_edges();
  int32_t n = prob.num_nodes();
  ...
  for (auto e : graph.edges()) {
    int32_t u = graph.edge_source(e);
    int32_t v = graph.edge_target(e);
    if ((u == 0 && v == 1) || (u == 1 && v == 3)) x_values[e] = 1.0;
  }
...
TEST_CASE("SeparationOracle: max_cuts_per_separator 0 means unlimited",
          "[oracle]") {
  auto prob = make_small_problem();
  int32_t m = prob.num_edges();
  int32_t n = prob.num_nodes();
...
TEST_CASE("BoundPropagator: has_all_pairs_bounds initially false",
          "[propagator]") {
  cptp::Problem prob;
  std::vector<cptp::Edge> edges = {{0, 1}, {0, 2}, {1, 2}};
...
TEST_CASE("BoundPropagator: sweep with loose UB fixes nothing",
          "[propagator]") {
  cptp::Problem prob;
  std::vector<cptp::Edge> edges = {{0, 1}, {0, 2}, {1, 2}};
  std::vector<double> costs = {1, 1, 100};
...
TEST_CASE("BoundPropagator: sweep_nodes no fixings when edges are open",
          "[propagator]") {
  cptp::Problem prob;
  std::vector<cptp::Edge> edges = {{0, 1}, {0, 2}, {1, 2}};
  std::vector<double> costs = {1, 1, 1};
...
TEST_CASE("BoundPropagator: propagate_fixed_edge does nothing with loose UB",
          "[propagator]") {
  cptp::Problem prob;
  std::vector<cptp::Edge> edges = {{0, 1}, {0, 2}, {0, 3},
                                   {1, 2}, {1, 3}, {2, 3}};
