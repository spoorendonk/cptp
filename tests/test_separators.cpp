...
TEST_CASE("SEC separator for s-t path: feasible path has no violations",
          "[sec][path]") {
  auto prob = make_small_path_problem();
  int32_t m = prob.num_edges();
  int32_t n = prob.num_nodes();
...
TEST_CASE("SEC path: rhs_coeff=1 when target in S (target_in_S=true)",
          "[sec][path]") {
...
TEST_CASE("Demand reachability: tour eliminates unreachable node",
          "[preprocess]") {
...
TEST_CASE("Demand reachability: path uses bidirectional check",
          "[preprocess][path]") {
...
TEST_CASE("Demand reachability: path eliminates unreachable node",
          "[preprocess][path]") {
...
TEST_CASE("Edge elimination: path uses bidirectional labeling",
          "[preprocess][path]") {
...
TEST_CASE("Primal heuristic: path produces valid open path",
          "[heuristic][path]") {
  auto prob = make_small_path_problem();
  auto result = cptp::heuristic::build_initial_solution(prob, 50);
...
TEST_CASE("LP-guided heuristic: reduces graph and finds solution",
          "[heuristic]") {
  auto prob = make_small_problem();
  int32_t m = prob.num_edges();
  int32_t n = prob.num_nodes();
  ...
  auto result = cptp::heuristic::lp_guided_heuristic(prob, x_lp, y_lp,
                                                     incumbent, 50.0, 0);

  REQUIRE(result.objective < std::numeric_limits<double>::max());
  REQUIRE(result.col_values.size() == static_cast<size_t>(m + n));
  REQUIRE(result.col_values[m + prob.source()] == 1.0);
}
...
TEST_CASE("RCI separator on fractional solution with high demands", "[rci]") {
  ...
  cptp::sep::RCISeparator rci;
  auto cuts = rci.separate(ctx);
  // RCI may or may not find cuts on this particular solution, but should not
  // crash
  REQUIRE(true);
}
...
TEST_CASE("Demand reachability: large capacity makes all reachable",
          "[preprocess]") {
  auto prob = make_small_problem();
...
TEST_CASE("Warm-start: deterministic mode produces identical results",
...
TEST_CASE("Warm-start: deterministic path produces identical results",
...
TEST_CASE("Warm-start: time budget option is deterministic no-op",
          "[heuristic][determinism]") {
  auto prob = make_small_problem();
