...
double held_karp(int32_t source, int32_t target,
                 const std::vector<int32_t>& nodes, std::span<const double> ap,
                 int32_t n) {
  ...
  if (k == 2) {
    double lb1 =
        d(source, nodes[0]) + d(nodes[0], nodes[1]) + d(nodes[1], target);
    double lb2 =
        d(source, nodes[1]) + d(nodes[1], nodes[0]) + d(nodes[0], target);
  ...
  for (int32_t last = 0; last < k; ++last) {
    double cost =
        dp[static_cast<size_t>(full) * k + last] + d(nodes[last], target);
...
double compute_set_lb(const Problem& prob, std::span<const double> all_pairs,
...
void greedy_grow(const Problem& prob, std::span<const double> all_pairs,
                 double ub, std::vector<int32_t>& set,
...
void greedy_shrink(const Problem& prob, std::span<const double> all_pairs,
                   double ub, std::vector<int32_t>& set,
                   std::span<const double> y_values, size_t min_size = 2) {
  bool improved = true;
  while (improved && set.size() > min_size) {
    ...
    for (size_t i = 0; i < set.size(); ++i) {
      ...
      if (lb > ub + 1e-6) {
        ...
        if (lb > best_lb + 1e-9 ||
            (lb > best_lb - 1e-9 &&
             y_values[set[i]] < y_values[set[best_idx]])) {
          best_lb = lb;
          best_idx = i;
          improved = true;
        }
...
void lift_cut(const Problem& prob, std::span<const double> all_pairs, double ub,
              const std::vector<int32_t>& set, std::span<const double> y_values,
              int32_t source, int32_t target, bool is_tour, double tol,
...
std::vector<Cut> SPISeparator::separate(const SeparationContext& ctx) {
  ...
  for (size_t ai = 0; ai < cands.size(); ++ai) {
    ...
    for (size_t aj = ai + 1; aj < cands.size(); ++aj) {
      ...
      if (lb > ub + 1e-6) {
        ...
        lift_cut(prob, ctx.all_pairs, ub, base_set, ctx.y_values, source,
                 target, is_tour, tol, lifted_nodes, lifted_coeffs);
        ...
        if (cut.violation > tol) cuts.push_back(std::move(cut));
  ...
  for (size_t si = 0; si < num_seeds; ++si) {
    ...
    if (static_cast<int32_t>(set.size()) <= 2)
    ...
    std::vector<int32_t> lifted_nodes;
    std::vector<double> lifted_coeffs;
    lift_cut(prob, ctx.all_pairs, ub, set, ctx.y_values, source, target,
             is_tour, tol, lifted_nodes, lifted_coeffs);
