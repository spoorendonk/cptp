...
std::vector<Cut> CombSeparator::separate(const SeparationContext& ctx) {
  ...
  for (double threshold : {0.5, 0.3}) {
    ...
    // stable_sort preserves deterministic edge-iteration order for equal
    // contributions.
    std::stable_sort(
        teeth.begin(), teeth.end(),
        [](const Tooth& a, const Tooth& b) { return a.contrib > b.contrib; });
    ...
    for (const auto& tooth : teeth) {
      ...
      if (used_inside[in_key] || used_outside[tooth.outside]) continue;
    ...
    for (auto e : graph.edges()) {
      int32_t u = graph.edge_source(e);
      int32_t v = graph.edge_target(e);
      if (!is_tour && ((u == depot && v == path_target) ||
