...
/// For tours: subtract depot profit once (depot appears at both ends but counts
/// once). For paths: subtract source and target profits.
inline double tour_objective(const Problem& prob,
                             const std::vector<int32_t>& tour) {
  double cost = 0.0;
  for (size_t i = 0; i + 1 < tour.size(); ++i)
    cost += edge_cost(prob, tour[i], tour[i + 1]);
  for (size_t i = 1; i + 1 < tour.size(); ++i) cost -= prob.profit(tour[i]);
  if (prob.is_tour()) {
...
inline void greedy_insert(const Problem& prob, std::vector<int32_t>& tour,
...
inline int local_search(const Problem& prob, std::vector<int32_t>& tour,
                        std::vector<bool>& in_tour, double& remaining_cap,
                        const std::vector<bool>& edge_active = {},
  ...
  for (int iter = 0; iter < max_iter_per_start; ++iter) {
      ...
      for (int32_t i = 1; i + cl < len - 1 && !improved; ++i) {
        ...
        for (int32_t j = 1; j < len - 1 && !improved; ++j) {
          ...
          if (insert_add - remove_save < -kLsEpsImprove) {
            std::vector<int32_t> seg(tour.begin() + i, tour.begin() + se + 1);
    ...
    for (int32_t i = 1; i < len - 1 && !improved; ++i) {
      ...
      for (int32_t in = 0; in < n && !improved; ++in) {
        ...
        const double delta = (new_prev + new_next - old_prev - old_next) -
...
inline bool init_seed_state(const Problem& prob, std::span<const int32_t> tour,
                            std::vector<bool>& in_tour, double& remaining_cap) {
  const int32_t n = prob.num_nodes();
...
inline bool perturb_swap_kick(const Problem& prob, std::vector<int32_t>& tour,
                              std::vector<bool>& in_tour, double& remaining_cap,
                              const std::vector<bool>& edge_active,
  ...
  for (int32_t trial = 0; trial < kKickTrials; ++trial) {
    const uint64_t key = deterministic_kick_key(start_index, round_index, trial,
...
inline bool perturb_relocate_kick(const Problem& prob,
  ...
  for (int32_t trial = 0; trial < kKickTrials; ++trial) {
    ...
    const int32_t from =
        1 + static_cast<int32_t>(key % static_cast<uint64_t>(interior_count));
    const int32_t to =
        1 + static_cast<int32_t>((key >> 32) % static_cast<uint64_t>(len - 1));
...
/// When edge_active is non-empty, only active edges are considered.
inline std::pair<std::vector<int32_t>, double> single_restart(
  ...
  while (static_cast<int32_t>(tour.size()) <= min_construction_size &&
         !customers.empty()) {
...
/// feasible tour/path.
inline std::vector<int32_t> fallback_feasible(
...
inline std::vector<TourCandidate> enumerate_small_seed_candidates(
    const Problem& prob, const std::vector<int32_t>& customers,
    const std::vector<bool>& edge_active = {}) {
  const int32_t source = prob.source();
  const int32_t target = prob.target();
  ...
  auto try_seed = [&](std::vector<int32_t> seed, double used_demand) {
    ...
    candidates.push_back(
        TourCandidate{.tour = std::move(seed), .objective = kNoSolution});
  };
...
inline void validate_tour_or_throw(const Problem& prob,
  ...
  if (tour.front() != source || tour.back() != target) {
    throw std::runtime_error(
        "Heuristic produced invalid route: wrong endpoints");
  }
  ...
  for (size_t i = 0; i < tour.size(); ++i) {
    ...
    if (v < 0 || v >= n) {
      throw std::runtime_error(
          "Heuristic produced invalid route: node id out of bounds");
    }
    if (i > 0 && i + 1 < tour.size()) {
      ...
      if (seen[static_cast<size_t>(v)]) {
        throw std::runtime_error(
            "Heuristic produced invalid route: repeated customer");
      }
  ...
  if (used > prob.capacity() + kLsEpsImprove) {
    throw std::runtime_error(
        "Heuristic produced invalid route: capacity violation");
  }
...
inline std::vector<double> tour_to_solution(const Problem& prob,
  ...
  if (static_cast<int32_t>(tour.size()) >= min_valid_size) {
    for (size_t i = 1; i + 1 < tour.size(); ++i) sol[m + tour[i]] = 1.0;
    for (size_t i = 0; i + 1 < tour.size(); ++i) {
...
/// Include edges with x_e > threshold + edges between pairs of nodes with y_i >
/// node_threshold.
inline ReducedGraph reduce_lp_threshold(const Problem& prob,
  ...
  for (auto e : g.edges()) {
    ...
    if (x_lp[e] > edge_threshold || (rg.node_active[u] && rg.node_active[v])) {
...
inline ReducedGraph reduce_rins(const Problem& prob,
  ...
  for (int32_t i = 0; i < n; ++i) {
    if (incumbent[m + i] > 0.5 || y_lp[i] > 0.5) rg.node_active[i] = true;
  }
...
inline ReducedGraph reduce_neighborhood(const Problem& prob,
  ...
  for (auto e : g.edges()) {
    int32_t u = g.edge_source(e);
    int32_t v = g.edge_target(e);
    if (x_lp[e] > seed_threshold || (rg.node_active[u] && rg.node_active[v])) {
      rg.edge_active[e] = true;
    }
  }
...
inline std::vector<bool> build_edge_activity_from_bounds(
  ...
  if (!fwd_bounds.empty() && !bwd_bounds.empty() &&
    ...
    auto eliminated = preprocess::edge_elimination(prob, fwd_vec, bwd_vec,
...
inline ConstructionPool build_construction_pool(
  ...
  pool.candidates = detail::enumerate_small_seed_candidates(prob, customers,
                                                            pool.edge_active);
  if (pool.candidates.empty()) {
    ...
    if (!fallback.empty()) {
      ...
      pool.candidates.push_back(detail::TourCandidate{
          .tour = std::move(fallback), .objective = fallback_obj});
    }
  }
  if (max_candidates > 0 &&
...
inline HeuristicResult run_local_search_from_pool(
  ...
  const bool progress_enabled =
      static_cast<bool>(progress_cb) && progress_opts != nullptr;
  const int64_t report_every =
      progress_enabled
  ...
  auto build_progress_snapshot = [&](bool final) -> WarmStartProgressSnapshot {
    ...
    snap.ls_iterations_total =
        ls_iterations_total.load(std::memory_order_relaxed);
  ...
  auto finalize_start = [&](bool ub_improved, int64_t ls_iters_for_start) {
    ls_iterations_total.fetch_add(ls_iters_for_start,
    ...
    if (progress_enabled && report_every > 0) {
      while (true) {
        ...
        if (next_report_milestone.compare_exchange_weak(
                milestone, milestone + report_every, std::memory_order_relaxed,
                std::memory_order_relaxed)) {
          periodic_due = true;
          break;
        }
    ...
    if ((progress_enabled && progress_opts->report_on_ub_improvement &&
         ub_improved) ||
        periodic_due) {
      emit_progress(false);
    }
...
inline HeuristicResult build_initial_solution(
  ...
  auto pool = build_construction_pool(prob, candidate_cap, fwd_bounds,
...
/// strategy: 0 = all (default), 1 = LP-threshold only, 2 = RINS only, 3 =
/// neighborhood only Returns improved solution, or empty col_values if none
/// found.
inline HeuristicResult lp_guided_heuristic(
  ...
  if (strategy == 0 || strategy == 1)
    graphs.push_back(reduce_lp_threshold(prob, x_lp, y_lp, lpg_edge_threshold,
                                         lpg_node_threshold));
  if (strategy == 0 || strategy == 2)
    graphs.push_back(
        reduce_rins(prob, x_lp, y_lp, incumbent, lpg_lp_threshold));
  if (strategy == 0 || strategy == 3)
    graphs.push_back(reduce_neighborhood(prob, x_lp, y_lp, lpg_seed_threshold));
  ...
  // LP-weighted ordering: sort by y_lp descending, breaking ties by
  // profit/demand
  auto make_lp_order = [&](const std::vector<int32_t>& customers) {
    ...
    std::sort(order.begin(), order.end(), [&](int32_t a, int32_t b) {
      ...
      double ra =
          prob.demand(a) > 0 ? prob.profit(a) / prob.demand(a) : prob.profit(a);
      double rb =
          prob.demand(b) > 0 ? prob.profit(b) / prob.demand(b) : prob.profit(b);
      return ra > rb;
    });
    return order;
  };
  ...
  parallel::parallel_for(0, total_restarts, [&](int r) {
    ...
    detail::TourCandidate best_local{.tour = std::move(tour), .objective = obj};

    const int32_t ils_start =
        static_cast<int32_t>(restart_seed + static_cast<uint32_t>(r));
    for (int32_t round = 0; round < detail::kIlsRounds; ++round) {
      std::vector<int32_t> kicked = best_local.tour;
      std::vector<bool> kicked_in_tour;
      double kicked_remaining_cap = 0.0;
      ...
      if (!detail::perturb_deterministic(prob, kicked, kicked_in_tour,
                                         kicked_remaining_cap, ea, ils_start,
                                         round)) {
        continue;
      }
      ...
      detail::local_search(prob, kicked, kicked_in_tour, kicked_remaining_cap,
                           ea, kDefaultLsMaxIterPerStart);
      const double kicked_obj = detail::tour_objective(prob, kicked);
      if (kicked_obj + detail::kLsEpsTie < best_local.objective ||
          (std::abs(kicked_obj - best_local.objective) <= detail::kLsEpsTie &&
           kicked < best_local.tour)) {
        best_local.tour = std::move(kicked);
        best_local.objective = kicked_obj;
      }
    ...
    results[static_cast<size_t>(r)] = {std::move(best_local.tour),
