...
int local_search(const Problem& prob, std::vector<int32_t>& tour,
  ...
  for (int iter = 0; iter < max_iter_per_start; ++iter) {
    ...
    // --- Node drop ---
    for (int32_t i = 1; i < len - 1 && len >= min_drop_size && !improved;
         ++i) {
      int32_t c = tour[i];
...
std::pair<std::vector<int32_t>, double> single_restart(
    ...
    const std::vector<int32_t>& order, const std::vector<bool>& edge_active) {
  const int32_t n = prob.num_nodes();
  const int32_t source = prob.source();
  const int32_t target = prob.target();
...
std::vector<int32_t> fallback_feasible(const Problem& prob,
                                       const std::vector<bool>& edge_active) {
  const int32_t n = prob.num_nodes();
  const int32_t source = prob.source();
  const int32_t target = prob.target();
...
std::vector<bool> build_edge_activity_from_bounds(
    ...
    std::span<const double> bwd_bounds, double correction,
    double upper_bound) {
  const int32_t n = prob.num_nodes();
  const int32_t m = prob.num_edges();
...
ConstructionPool build_construction_pool(
    const Problem& prob, int max_candidates, std::span<const double> fwd_bounds,
    std::span<const double> bwd_bounds, double correction,
    double upper_bound) {
...
HeuristicResult run_local_search_from_pool(
    ...
    int ls_max_iter_per_start, int32_t max_workers,
    const std::vector<bool>& edge_active_override,
    const WarmStartProgressOptions* progress_opts,
    const WarmStartProgressCallback& progress_cb,
    bool reuse_candidates_for_starts) {
  if (pool.candidates.empty()) {
...
HeuristicResult build_initial_solution(
    const Problem& prob, int num_restarts, double time_budget_ms,
    std::span<const double> fwd_bounds, std::span<const double> bwd_bounds,
    double correction, double upper_bound, int32_t max_workers) {
...
HeuristicResult lp_guided_heuristic(
    ...
    double time_budget_ms, int strategy, int max_restarts,
    uint32_t restart_seed, double lpg_edge_threshold,
    double lpg_node_threshold, double lpg_lp_threshold,
    double lpg_seed_threshold) {
  const int32_t n = prob.num_nodes();
  const int32_t source = prob.source();
  const int32_t target = prob.target();
  const double Q = prob.capacity();
